  ...
  try {
    ...
    const bilingualTours = [
      {
        ...
        meetingPoint: { address: 'Old Town Square, near the Astronomical Clock', mapUrl: 'https://maps.google.com/?q=50.0870,14.4210', instructions: 'Meet at the statue of Jan Hus in the center of Old Town Square.' },
      ...
      },
      {
        ...
        ru: { title: 'Пражский Град и Градчаны', slug: 'prazhskij-grad-i-gradchany', excerpt: 'Пешеходная экскурсия по левому берегу: Собор Святого Вита, Злата уличка, сады, спуск по Нерудовой улице.', description: richText('Исследуйте величественный Пражский Град — крупнейший древний замковый комплекс в мире. Эта трёхчасовая индивидуальная экскурсия проведёт вас через потрясающий Собор Святого Вита, по очаровательной Златой уличке, через Королевский сад и вниз через живописную Малую Страну с её барочной архитектурой и скрытыми жемчужинами.') },
        ...
        meetingPoint: { address: 'Prague Castle main entrance, Hradcanske namesti', mapUrl: 'https://maps.google.com/?q=50.0909,14.3986', instructions: 'Meet at the main gate of Prague Castle (Hradcanske namesti).' },
      ...
      },
      {
        ...
        meetingPoint: { address: 'Your hotel or any location in Prague center', mapUrl: 'https://maps.google.com/?q=50.0755,14.4378', instructions: 'Hotel pickup included. Please provide your hotel address when booking.' },
    ...
    for (const tour of bilingualTours) {
      ...
      const data: Record<string, unknown> = {
        ...
        pricing: {
          model: 'GROUP_TIERS',
          groupTiers: [{ minGuests: 1, price: tour.groupPrice }],
        },
    ...
    for (let i = 0; i < ruOnlyTours.length; i++) {
      ...
      const data: Record<string, unknown> = {
        ...
        pricing: {
          model: 'GROUP_TIERS',
          groupTiers: [{ minGuests: 1, price: tour.price }],
        },
        publishedLocales: ['ru'],
        status: 'published',
        sortOrder: tour.sortOrder,
